...
@section('content')
@include('Penawaran_qsend.create')

<div class="nk-block nk-block-lg">
    <div class="nk-block-head nk-block-head-sm">
        <div class="nk-block-between">
            <div class="nk-block-head-content">
                <h3 class="nk-block-title page-title">Data Penawaran Q-Send</h3>
            ...
            <tbody class="tb-odr-body">
                ...
                <tr class="tb-odr-item">
                    ...
                    <button type="submit" class="btn btn-warning" data-toggle="modal" data-target=".bs-example-modal-lg" onclick="updatePenawaran('{{$data->id}}', '{{$data->penawaran}}','{{$data->gambar_penawaran}}');">Ubah</button>
                        <a href="/deletePenawaranQsend/{{ $data->id }}" class="btn btn-danger" onclick="if (confirm('Hapus Item Terpilih?')){return true;}else{event.stopPropagation(); event.preventDefault();};">Hapus</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
...
<script>
    ...
    function updatePenawaran($a, $b) {
        $('#link_url').attr('action', '{{url('/updatePenawaranQsend')}}');
        $("#id_penawaran").val($a);
        $("#penawaran").val($b);
    }
